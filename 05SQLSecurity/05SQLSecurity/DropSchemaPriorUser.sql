/*
	drop a schema 
*/
-- 1. get a list of objects the user have (schema)
SELECT schema_name(schema_id) as schema_name, name , type, type_desc
FROM sys.objects 
WHERE schema_id = SCHEMA_ID('myuser')

-- 2. script them and drop them one by one by script
